...
type OTPMailer interface {
	SendOTP(otpCode string, receiver string) error
}
...
const otpEmailTemplate = `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
	...
	<div style="float:right;padding:8px 0;color:#aaa;font-size:0.8em;line-height:1;font-weight:300">
		<p>Copyright &copy; Go Rest boilerplate %d</p>
...
func (mailer *otpMailer) SendOTP(otpCode string, receiver string) error {
	now := time.Now()
	emailBody := fmt.Sprintf(otpEmailTemplate, otpCode, now.Year())
